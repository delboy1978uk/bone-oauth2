...
{
    private function getMiddleware($resourceServer)
    {
        $clientRepo = $this->createMock(ClientRepository::class);
        $clientService = $this->createMock(ClientService::class);
        $clientService->method('getClientRepository')->willReturn($clientRepo);
        $client = new Client();
        $client->setUser(new \Del\Entity\User());
    ...
    public function testProcessWithValidToken()
    {
        ...
        $middleware = $this->getMiddleware($resourceServer);
